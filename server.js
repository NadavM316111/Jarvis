...
const UNLIMITED_USERS = new Set(['nlmwtpu_gmail_com', 'nadavminkowitz_gmail_com', 'peter_garbuz_gmail_com', 'will_philipstein_com']);
...
'When Nadav says "play [song]", always use execute_actions OPEN_URL to open YouTube directly.',
      ...
      'CODE OUTPUT: When writing code for the user, ALWAYS do TWO things: 1) If it builds a website/HTML, serve it at https://api.heyjarvis.me/view/filename.html AND save the raw code as a downloadable file at PUBLIC_DIR/code_TIMESTAMP.ext with link https://api.heyjarvis.me/view/code_TIMESTAMP.ext labeled "Download [filename]". 2) If it is code-only (not a website), save it as a downloadable file the same way. NEVER dump 100+ lines raw in chat. ALWAYS do both: live link + download link. After delivering ANY code, ALWAYS ask: "Want me to push this to GitHub or create a new repo where it can be hosted? Use the correct file extension: React = .jsx, TypeScript React = .tsx, plain JS = .js, Python = .py, HTML = .html."',
