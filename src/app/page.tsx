  ...
  if (codeFileMatch) {
    ...
    extras.push(
      <div key="code-download" style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
        <a href={url.replace('/view/', '/download/')} download={filename}
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/auth/github/status`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json()).then(d => setGithubConnected(d.connected)).catch(() => {});
...
<button
  onClick={async () => {
    const url = `${API}/auth/github?token=${token}`;
    const invoke = (window as any).__TAURI_INVOKE__;
    if (invoke) {
      await invoke('execute_shell', { command: `open "${url}"` });
    } else {
  ...
  className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl border text-xs font-medium transition-all mb-2 ${githubConnected ? 'bg-orange-500/15 border-orange-500/30 text-orange-400' : 'border-black/10 hover:border-black/20'}`}
  style={!githubConnected ? { color: 'var(--jarvis-text-sub)', background: 'var(--jarvis-card)' } : undefined}
>
